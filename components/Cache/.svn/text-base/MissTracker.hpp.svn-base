...
using namespace boost::multi_index;
  ...
  #define DBG_DeclareCategories MissTracking
  #include DBG_Control()

#ifdef FLEXUS_TRACK_COHMISSES
  Flexus::Stat::StatInstanceCounter<long long> theCoherenceMisses_User("sys-CoherenceMisses:User");
  Flexus::Stat::StatInstanceCounter<long long> theCoherenceMisses_System("sys-CoherenceMisses:System");
...
namespace nCache {
  ...
  struct MissTracker {
    ...
    //Miss Tracking
      ...
      unsigned long long theLastAccounting_Miss;
      unsigned long long theLastAccounting_NonCoherence;
      unsigned long long theLastAccounting_Coherence;
      unsigned long long theLastStart_Miss;
      unsigned long long theLastStart_NonCoherence;
      unsigned long long theLastStart_Coherence;
      unsigned int theCount_Miss;
      unsigned int theCount_NonCoherence;
      unsigned int theCount_Coherence;

      long long theLastCoherenceAddress;
    ...
    //Stats
      ...
      Stat::StatInstanceCounter<long long> theCyclesWithN_Miss;
      Stat::StatInstanceCounter<long long> theCyclesWithN_NonCoherence;
      Stat::StatInstanceCounter<long long> theCyclesWithN_Coherence;
        ...
        switch( front->theType ) {
          ...
          case kStart_Coherence:
            ...
            if (front->theTracker->address() ) {
              long long address = *front->theTracker->address() & (~ 63LL);
              if ( address == theLastCoherenceAddress) {
                ...
                if (front->theTracker->OS() && *front->theTracker->OS()) {
                  ...
                  DBG_( Trace, AddCat(MissTracking) ( << theName << " CONTENTION Miss to " << std::hex << address << std::dec  << "[S]" ));
                } else {
                  ++theMisses_U;
                  ++theMisses_U_CohContend;
                  DBG_( Trace, AddCat(MissTracking) ( << theName << " CONTENTION Miss to " << std::hex << address << std::dec  << "[U]" ));
                }
            ...
            if (front->theTracker->speculativeAtomicLoad() && *front->theTracker->speculativeAtomicLoad()) {
              if (front->theTracker->OS() && *front->theTracker->OS()) {
                ...
                DBG_( Trace, AddCat(MissTracking) ( << theName << " SPEC-ATOMIC-COHERENCE Miss to " << std::hex << ((*front->theTracker->address() & (~ 63LL))) << std::dec  << "[S]" ));
              } else {
                ++theMisses_U;
                ++theMisses_U_SpecAtomicCoherence;
                DBG_( Trace, AddCat(MissTracking) ( << theName << " SPEC-ATOMIC-COHERENCE Miss to " << std::hex << ((*front->theTracker->address() & (~ 63LL))) << std::dec  << "[U]" ));
              }
            } else {
              if (front->theTracker->OS() && *front->theTracker->OS()) {
                ++theMisses_S;
                ...
                #endif //FLEXUS_TRACK_COHMISSES
                DBG_( Trace, AddCat(MissTracking) ( << theName << " COHERENCE Miss to " << std::hex << ((*front->theTracker->address() & (~ 63LL))) << std::dec  << "[S]" ));
              } else {
                ++theMisses_U;
                ...
                #endif //FLEXUS_TRACK_COHMISSES
                DBG_( Trace, AddCat(MissTracking) ( << theName << " COHERENCE Miss to " << std::hex << ((*front->theTracker->address() & (~ 63LL))) << std::dec  << "[U]" ));
              }
            }
