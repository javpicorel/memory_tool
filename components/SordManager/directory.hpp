...
class Directory {
   ...
   BaseSORDManager & theSORDManager;
   bool theSkipOS;

  public:
    Directory(BaseSORDManager & aSordManager, bool useOS)
      : theSORDManager( aSordManager )
      , theSkipOS( !useOS )
      {}
    ...
    void downgrade( tID aProducer, tAddress anAddress, bool isOS) {
      ...
      if(block) {
        theSORDManager.dirFlush(aProducer, anAddress);
        ...
        if (block->state == StateExclusive) {
          if (block->producer == aProducer) {
            ...
            theSORDManager.writeMRP( block->producer, anAddress, isOS );
            theSORDManager.produce( aProducer, anAddress );
    ...
    void storeMiss( tID aProducer, tAddress anAddress, bool isOS) {
      ...
      if(block) {
        ...
        if(block->state == StateShared) {
          ...
          theSORDManager.invalidate(aProducer, anAddress);
          theSORDManager.dirUpgrade(aProducer, anAddress);
        }
          ...
          if(aProducer != block->producer) {
            ...
            theSORDManager.dirProduction(block->producer, anAddress);  // count the old producer
            theSORDManager.writeMRP( block->producer, anAddress, isOS );
            theSORDManager.dirUpgrade(aProducer, anAddress);
          ...
          if (block->producer == aProducer) {
            ...
            theSORDManager.mispredict(aProducer, anAddress);
          }
          else {
            // now we know the downgrade was a real production
            theSORDManager.dirProduction(block->producer, anAddress);
            block->producer = aProducer;
            theSORDManager.dirUpgrade(aProducer, anAddress);
          }
          block->state = StateExclusive;
      ...
      if(block) {
        ...
        if(block->state == StateShared) {
          ...
          if (! block->isSharer(aConsumer) ) {
            ...
            if (! wasPredicted) {
              theSORDManager.readMRP( aConsumer, anAddress, isOS);
              theSORDManager.consume(aConsumer, anAddress);
              theSORDManager.dirConsumption(aConsumer, anAddress);
            }
