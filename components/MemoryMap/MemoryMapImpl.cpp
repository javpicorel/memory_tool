...
typedef unsigned int node_id_t;
  ...
  struct MemoryMapFactory {
    ...
    MemoryMapFactory() { DBG_Assert( (theMemoryMapFactory == 0) ); theMemoryMapFactory = this; }
...
struct MemoryPage {
  private:
    typedef unsigned long access_count_array[MemoryMap::NumAccessTypes];
  ...
  public:
    ...
    void initialize(node_id_t aNumCPUs) {
      ...
      for(unsigned int i = 0; i < MemoryMap::NumAccessTypes; ++i) {
        for(unsigned int j = 0; j < aNumCPUs + 1; ++j) {
...
std::ostream & operator<<(std::ostream & anOstream, MemoryPage const & aMemoryPage) {
  for (int i = 0; i < MemoryMap::NumAccessTypes; ++i) {
    anOstream << "  " << MemoryMap::AccessType_toString(MemoryMap::AccessType(i)) << ':';
    for (unsigned int j = 0; j < aMemoryPage.theNumCPUs + 1; ++j) {
