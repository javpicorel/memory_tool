...
struct OutfileRecord {
  ...
  OutfileRecord(std::string const & aName, int aNumProcs)
    : theBaseName(aName)
...
OutfileRecord * theL2Accesses = 0;
OutfileRecord * theOffChipAccesses = 0;
...
void initialize( std::string const & aPath ) {
  DBG_(Dev, ( << "Creating output file" ) );
  theL2Accesses= new OutfileRecord( aPath + "/L2_trace", 1);
  theL2Accesses->init();
  theOffChipAccesses = new OutfileRecord( aPath + "/OC_trace", 1);
...
void addL2Access(TraceData const & theRecord) {
  DBG_(Verb, (  << "C[" << theRecord.aNode << "] 0x" << std::hex << theRecord.anAddress
                  << " PC:" << theRecord.aPC << std::dec << ( theRecord.theOS ? "s " : "u " )
               ) );
