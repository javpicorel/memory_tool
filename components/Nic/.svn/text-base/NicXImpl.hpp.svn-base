...
private:
  ...
  void msgFromNode() {
    ...
    for(int ii = 0; ii <  FLEXUS_NIC_NUMPORTS; ++ii) {
      if(!sendQueue[ii].empty()) {
        if (static_cast<unsigned int>(sendQueue[ii].front()[NetworkMessageTag]->dest) == flexusIndex()) {
...
FLEXUS_PORT_ARRAY_WIDTH( NicX, ToNetwork ) { return cfg.VChannels; }
FLEXUS_PORT_ARRAY_WIDTH( NicX, FromNetwork ) { return cfg.VChannels; }
