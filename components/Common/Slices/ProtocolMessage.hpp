...
struct ProtocolMessage : public boost::counted_base {
  ...
  public:
    long long     theInputQTimestamp;

  public:
    //Network packets
    ProtocolMessage(tAddress anAddress, unsigned long aDest, tVC aVC, node_id_t aSrc, node_id_t aRequester, Protocol::ProtocolMessageType aMessageType, unsigned anInvalidationCount, bool anAnyInvalidations, bool isPrefetch, boost::intrusive_ptr<TransactionTracker> aTracker);
    ...
    node_id_t requester() const { DBG_Assert( ! theLocal ); return theRequester; }
    node_id_t respondent() const { DBG_Assert( ! theLocal ); return theSource; }
    node_id_t source() const { DBG_Assert( ! theLocal ); return theSource; }
    node_id_t dest() const { DBG_Assert( ! theLocal ); return theDest; }
    node_id_t invalidationCount() const { DBG_Assert( ! theLocal ); return theInvalCount; }
    bool anyInvalidations() const { DBG_Assert( ! theLocal ); return theAnyInvalidations; }
