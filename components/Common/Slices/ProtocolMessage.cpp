...
ProtocolMessage::ProtocolMessage(tAddress anAddress, unsigned long aDest, tVC aVC, node_id_t aSrc, node_id_t aRequester, Protocol::ProtocolMessageType aMessageType, unsigned anInvalidationCount, bool anAnyInvalidations, bool isPrefetch, boost::intrusive_ptr<TransactionTracker> aTracker)
      : theLocal(false)
...
boost::intrusive_ptr<DirectoryEntry const> ProtocolMessage::dirEntry() const { DBG_Assert( theLocal ); return theDirEntry; }
boost::intrusive_ptr<TransactionTracker> ProtocolMessage::transactionTracker() const { return theTracker; }
