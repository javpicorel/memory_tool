...
class FLEXUS_COMPONENT(ReuseDistance) {
  ...
  typedef unsigned long long tRefCount;
  ...
  class stdDevLog2Histogram {
   ...
   public:
    ...
    typedef std::pair<long long /*mean*/, value_type /*stdd*/> update_type;
    std::vector<value_type> theBuckets;
    std::vector<long long> theBucketCounts;
   ...
   private:
    ...
    double calculate(const value_type oldStdDev,
                     const long long  oldPopulation,
                     const value_type newStdDev,
                     const long long  newPopulation) const
    {
   ...
   public:
    ...
    stdDevLog2Histogram & operator << (const update_type anUpdate) {
      if (anUpdate.first == 0) {
        ...
        long long log = log2(anUpdate.first);
    ...
    void print() const {
      ...
      for (unsigned int i=1; i < theBuckets.size(); i++) {
  ...
  typedef struct tLineReuseDist {
    ...
    void update( const unsigned long long anUpdate ) {
    ...
    long long mean( void ) const {
      if (ReuseDist_k == 0) {
        return 0;
      } else {
        return static_cast<long long>(ReuseDist_Sum / static_cast<double>(ReuseDist_k));
      }
    }

    long long meanWithAging( const unsigned long long anUnusedDistance ) const {
      static const unsigned int ReuseFactor = 1UL;
      static const unsigned int UnuseFactor = 1UL;

      if ((ReuseDist_k == 0) && (UnuseFactor == 0)) {
        return 0;
      } else {
        return static_cast<long long>( (ReuseDist_Sum * ReuseFactor + anUnusedDistance * UnuseFactor)
                                       / static_cast<double>(ReuseDist_k*(ReuseFactor==0 ? 0 : 1) + (UnuseFactor==0 ? 0 : 1))   );
      }
    }

    long long samples( void ) const {
      return ReuseDist_k;
    }
  ...
  typedef struct {
    ...
    long long      sharersBitmask;           // # cores reading the block within the sharing window (or the DirtyShared window for RW blocks)
    short          coreIdxLastWriter;        // the core index of the last writer
    short          coreIdxLastRef;           // the core index of the last requestor
  ...
  unsigned long long theReuseWinSizeInCycles;
  unsigned long long theCurrReuseWinCycles;
  ...
  Stat::StatInstanceCounter<long long> theNumSharersRO;
  ...
  Stat::StatInstanceCounter<long long> theNumSharersRW;
  ...
  Stat::StatInstanceCounter<long long> cdfBlocksL2Refs;
  Stat::StatInstanceCounter<long long> cdfBlocksL2RefsPrivateData;
  Stat::StatInstanceCounter<long long> cdfBlocksL2RefsPrivateFetch;
  Stat::StatInstanceCounter<long long> cdfBlocksL2RefsROData;
  Stat::StatInstanceCounter<long long> cdfBlocksL2RefsROFetch;
  Stat::StatInstanceCounter<long long> cdfBlocksL2RefsRWData;
  Stat::StatInstanceCounter<long long> cdfBlocksL2RefsRWFetch;
 ...
 private:
  ...
  inline long long CoreIdx2SharerBit( const short coreIdx ) const {
    return (1LL << coreIdx);
  }

  inline bool isSharer( const long long sharersBitmask,
                        const short     coreIdx ) const
  {
    return ((sharersBitmask & CoreIdx2SharerBit(coreIdx)) != 0LL);
  }

  inline int countNumSharers( const long long sharersBitmask ) const {
    int count = 0;
    for (int i=0; i < (int) (sizeof(sharersBitmask)*8); i++) {
  ...
  bool isBlockPrivateD(const block_address_with_flags_t anAddressWithFlags) { return ((static_cast<unsigned long>(anAddressWithFlags) & 0x7UL) == 0x0UL); }
  bool isBlockPrivateI(const block_address_with_flags_t anAddressWithFlags) { return ((static_cast<unsigned long>(anAddressWithFlags) & 0x7UL) == 0x1UL); }
  bool isBlockROD     (const block_address_with_flags_t anAddressWithFlags) { return ((static_cast<unsigned long>(anAddressWithFlags) & 0x7UL) == 0x2UL); }
  bool isBlockROI     (const block_address_with_flags_t anAddressWithFlags) { return ((static_cast<unsigned long>(anAddressWithFlags) & 0x7UL) == 0x3UL); }
  bool isBlockRWD     (const block_address_with_flags_t anAddressWithFlags) { return ((static_cast<unsigned long>(anAddressWithFlags) & 0x7UL) == 0x4UL); }
  bool isBlockRWI     (const block_address_with_flags_t anAddressWithFlags) { return ((static_cast<unsigned long>(anAddressWithFlags) & 0x7UL) == 0x5UL); }
  bool isBlockAny     (const block_address_with_flags_t anAddressWithFlags) { return true; }

  void computeCDFBlocksL2Refs( const tReuseOrderedSet &               aReuseOrderedSet,
                               const tRefCount                        aTotalRefs,
                               Stat::StatInstanceCounter<long long> & aCdfBlocksRefs,
                               tIsBlockxxxFunc                        anIsBlockxxxFunc )
  {
    ...
    static const unsigned long pct_step   = 5;
    const unsigned long        ref_step   = aTotalRefs * pct_step / 100;
    unsigned long              ref_target = ref_step;

    tRefCount     currRefs   = 0;
    unsigned long currBlocks = 0;
 ...
 public:
  ...
  long long getMeanReuseDist(const PhysicalMemoryAddress aPhysAddress) {
  ...
  void finalizeWindow( void ) {
    ...
    */
    ...
    for (int i=0; i < lineStdDevReuseDistPCCorr.buckets(); ++i) {
      stdDevReuseDistPCCorr << std::make_pair( i ? (1LL << (i-1)) : 0,
                                               static_cast<long long>(lineStdDevReuseDistPCCorr.bucketVal(i)) );
    }
    ...
    for (tReuseMap::iterator iter = theReuseMap.begin(); iter != theReuseMap.end(); iter++) {
      ...
      // RO block
        ...
        int curr_sharers_count = countNumSharers(theReuseMapEntry.sharersBitmask);
    ...
    for (int i=0; i < lineStdDevReuseDistPrivateData.buckets(); ++i) {
      stdDevReuseDistPrivateData << std::make_pair( i ? (1LL << (i-1)) : 0,
                                                    static_cast<long long>(lineStdDevReuseDistPrivateData.bucketVal(i)) );
    }
    for (int i=0; i < lineStdDevReuseDistROData.buckets(); ++i) {
      stdDevReuseDistROData << std::make_pair( i ? (1LL << (i-1)) : 0,
                                               static_cast<long long>(lineStdDevReuseDistROData.bucketVal(i)) );
    }
    for (int i=0; i < lineStdDevReuseDistRWData.buckets(); ++i) {
      stdDevReuseDistRWData << std::make_pair( i ? (1LL << (i-1)) : 0,
                                               static_cast<long long>(lineStdDevReuseDistRWData.bucketVal(i)) );
    }
      
    for (int i=0; i < lineStdDevReuseDistPrivateFetch.buckets(); ++i) {
      stdDevReuseDistPrivateFetch << std::make_pair( i ? (1LL << (i-1)) : 0,
                                                     static_cast<long long>(lineStdDevReuseDistPrivateFetch.bucketVal(i)) );
    }
    for (int i=0; i < lineStdDevReuseDistROFetch.buckets(); ++i) {
      stdDevReuseDistROFetch << std::make_pair( i ? (1LL << (i-1)) : 0,
                                                static_cast<long long>(lineStdDevReuseDistROFetch.bucketVal(i)) );
    }
    for (int i=0; i < lineStdDevReuseDistRWFetch.buckets(); ++i) {
      stdDevReuseDistRWFetch << std::make_pair( i ? (1LL << (i-1)) : 0,
                                                static_cast<long long>(lineStdDevReuseDistRWFetch.bucketVal(i)) );
    }
  ...
  void insertReuseMapEntry( const MemoryMessage & aMessage,
                            ...
                            const short           coreIdx,      // the index of the core that issued the reference
  ...
  void push( interface::RequestIn const &, ReuseDistanceSlice & aReuseDistanceSlice )
  {
    ...
    const short           coreIdx  = static_cast<short>(aMessage.coreIdx()); // the index of the core that issued the reference
