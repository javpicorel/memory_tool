...
typedef unsigned long PhtIndex;
typedef unsigned long CptIndex;
typedef unsigned long CptTag;
typedef unsigned long BlockAddress;
typedef unsigned long GroupAddress;
typedef unsigned long GroupOffset;
typedef unsigned long GroupIndex;
...
long long theSpatialGroupCount = 0;

typedef unsigned long long RepetIndex;
struct SpatialGroupEntry {
  ...
  SpatialGroupEntry(SpatialPattern aPattern, SpatialPattern aFills)
    ...
    , unique_id( ++theSpatialGroupCount )
  {}
  SpatialGroupEntry(std::istream & ifs, unsigned long sgpBlocks)
    : pattern(sgpBlocks)
    , fills(sgpBlocks)
...
class SpatialHistorySingle {
  ...
  unsigned long long theLastAccess;
public:
  SpatialHistorySingle(SpatialPattern pattern, unsigned long long access)
    : theHistory(pattern)
    , theLastAccess(access)
  {}
  SpatialHistorySingle(std::istream & line, unsigned long sgpBlocks)
  ...
  void update(SpatialPattern pattern, unsigned long long access) {
    theHistory = pattern;
    theLastAccess = access;
  }
...
typedef flexus_boost_seq_map<unsigned char,unsigned char> OrderingMap;  // <BlockNo,SeqNo>
...
struct GroupOrderingEntry {
  ...
  unsigned char theNextSeqNo;
  GroupOrderingEntry()
    : theNextSeqNo(0)
  {}
  GroupOrderingEntry(unsigned char blockNo)
    : theNextSeqNo(1)
  {
    theOrder.push_back( std::make_pair(blockNo,0) );  // first seqNo is zero
  }
  void nextBlock(unsigned char blockNo) {
...
struct CurrentStreamEntry {
  ...
  unsigned char theBlockNo;
  short theHits;
  GroupOffset theOffset;
  bool theIsRight;
  CurrentStreamEntry(OrderingMap order, unsigned char lastBlock, GroupOffset offset)
    : theDense(false)
    , theOrder(order)
    ...
    , theOffset(offset)
  {}
  CurrentStreamEntry(SpatialPattern pattern, unsigned char nextBlock, GroupOffset offset, bool isRight)
    : theDense(true)
    , thePattern(pattern)
...
struct DoneStreamEntry {
  ...
  short thePossible;
  short theHits;
  DoneStreamEntry(bool dense, short possible, short hits)
    : theDense(dense)
...
struct GroupTimeEntry {
  ...
  unsigned long long lastEvict;
  unsigned long long lastBegin;
  unsigned long long lastEnd;
  GroupTimeEntry(unsigned long long curr)
    : live(true)
    , lastAccess(curr)
    , lastEvict(curr)
    , lastBegin(curr)
...
struct ActiveGroupEntry {
  ...
  ActiveGroupEntry(unsigned long curr, SpatialPattern initial)
    : start(curr)
    , recent(curr)
    , outstanding(initial)
  {}
...
public:
  SpatialPrefetcher( std::string statName, std::string altLoadName, int aNode )
    : theName(statName)
            ...
            int bufSize, int streamDescs, bool delayedCommits, int cptFilterSize)
  {
    ...
    if(theEnableUsage) {
      ...
      for(ii = 0; ii <= theMaxSgpBits; ii++) {
        ...
        theUsageStats.push_back(new Stat::StatInstanceCounter<long long>(name));
      }
    }

    if(theEnableOrdering) {
      DBG_Assert(theEnableRepet);
      DBG_Assert(theSgpBlocks <= 256);  // otherwise too big for unsigned char
    ...
    if(theEnableRepet) {
      ...
      if(thePhtAssoc > 0) {
        ...
        unsigned long theUsefulBottomBits;
        switch(theRepetType) {
          case 0:  // PC only
...
private:
  ...
  int theNodeId;
  unsigned long theNumAccesses;
  unsigned long long theCounter;

  // for spatial group usage
  std::vector<SpatialGroupUsage> theGroupUsages;
  std::vector<Stat::StatInstanceCounter<long long> * > theUsageStats;
  ...
  struct OrderedBlockEntry {
    ...
    bool dense;
    OrderedBlockEntry(unsigned long long curr, bool d)
      : cycles(curr)
      , dense(d)
    {}
  };
  ...
  typedef std::map<unsigned long,int> ActiveGenTime;  // this CANNOT be a hash_map!
  typedef ActiveGenTime::iterator ActiveGenIter;
  ActiveGenTime theActiveGenBegins;
  ActiveGenTime theActiveGenEnds;

  int theCurrPrefetchGen;
  long long theCurrPrefetchGenerationUUID;
  ...
  unsigned long theSgpBlocks;
  unsigned long theRepetType;
  bool theRepetFills;
  bool theSparseOpt;
  unsigned long thePhtSize;
  unsigned long thePhtAssoc;
  unsigned long thePcBits;
  unsigned long theCptType;
  unsigned long theCptSize;
  unsigned long theCptAssoc;
  long theCptFilterSize;
  bool theCptSparse;
  bool theFetchDist;
  long theStreamWindow;
  bool theStreamDense;
  bool theSendStreams;
  unsigned long theBufSize;
  unsigned long theStreamDescs;
  bool theDelayedCommits;

  long theCptSparseCount;
  bool theCurrDense;
  bool theStitchReady;

  unsigned long theBlockOffsetMask;
  unsigned long theBlockBits;
  unsigned long theSgpBits;   // bits necessary to encode block offset within a SG
  unsigned long theWordBits;  // bits necessary to encode word offset within a SG
  unsigned long theByteBits;  // bits necessary to encode byte offset within a SG
  unsigned long theWordInBlockBits;  // bits necessary to encode word offset within a block
  MemoryAddress thePCmask;
  unsigned long theMinSgpBits;
  unsigned long theMaxSgpBits;
  ...
  Stat::StatInstanceCounter<long long> statUsageAccess;
  Stat::StatInstanceCounter<long long> statUsageFill;
  Stat::StatInstanceCounter<long long> statUsageOffChip;
  ...
  Stat::StatInstanceCounter<long long> statRepetUsageAccess;
  Stat::StatInstanceCounter<long long> statRepetUsageFill;
  Stat::StatInstanceCounter<long long> statRepetUsageOffChip;
  Stat::StatInstanceCounter<long long> statRepetUsageCov;
  ...
  Stat::StatInstanceCounter<long long> statRepetEndGenNoIndexAccesses;
  Stat::StatInstanceCounter<long long> statRepetEndGenNoIndexMisses;
  Stat::StatInstanceCounter<long long> statRepetEvictCptNoIndexAccesses;
  Stat::StatInstanceCounter<long long> statRepetEvictCptNoIndexMisses;
  Stat::StatInstanceCounter<long long> statRepetLateIndexAccesses;
  Stat::StatInstanceCounter<long long> statRepetLateIndexMisses;
  Stat::StatInstanceCounter<long long> statRepetNextGenNoIndexAccesses;
  Stat::StatInstanceCounter<long long> statRepetNextGenNoIndexMisses;

  Stat::StatInstanceCounter<long long> statRepetPattern;
  Stat::StatInstanceCounter<long long> statRepetPatternDensity;
  Stat::StatInstanceCounter<long long> statRepetActualFillHits;
  Stat::StatCounter statRepetActualHitGens;
  Stat::StatInstanceCounter<long long> statParallelGroups;
  Stat::StatInstanceCounter<long long> statOrderDist;

  Stat::StatCounter statOrderBaseHitsDense;
  Stat::StatCounter statOrderBaseHitsSparse;
  Stat::StatInstanceCounter<long long> statOrderBaseLenDense;
  Stat::StatInstanceCounter<long long> statOrderBaseLenSparse;
  Stat::StatInstanceCounter<long long> statOrderDeltasDense;
  Stat::StatInstanceCounter<long long> statOrderDeltasSparse;
  Stat::StatInstanceCounter<long long> statOrderRightJumpsDense;
  Stat::StatInstanceCounter<long long> statOrderRightJumpsSparse;
  Stat::StatInstanceCounter<long long> statOrderLeftJumpsDense;
  Stat::StatInstanceCounter<long long> statOrderLeftJumpsSparse;
  Stat::StatInstanceCounter<long long> statOrderSameDirJumpsDense;
  Stat::StatInstanceCounter<long long> statOrderSameDirJumpsSparse;
  Stat::StatInstanceCounter<long long> statOrderDiffDirJumpsDense;
  Stat::StatInstanceCounter<long long> statOrderDiffDirJumpsSparse;
  ...
  Stat::StatInstanceCounter<long long> statBufFetchGenSize;
  ...
  Stat::StatInstanceCounter<long long> statStreamLenDense;
  Stat::StatInstanceCounter<long long> statStreamLenSparse;
  Stat::StatInstanceCounter<long long> statStreamLenPossibleDense;
  Stat::StatInstanceCounter<long long> statStreamLenPossibleSparse;
...
public:
  ...
  void commit(MemoryAddress addr, MemoryAddress pc, unsigned long long ltime) {
  ...
  void store(MemoryAddress addr, MemoryAddress pc, bool miss, bool priv, unsigned long long ltime) {
...
private:
  ...
  void finalizeGroupUsage(SpatialPattern pattern, int groupBits) {
      ...
      if(evict) {
        ...
        if(theEnableOrdering) {
          ...
          if(append) {
            OrderingBuildIter build = theOrderingBuild.find(group);
            DBG_Assert(build != theOrderingBuild.end());
            unsigned char offset = makeGroupOffset(addr, theSgpBits);
  ...
  std::pair<RepetReturnType, long long> doRepetCommit(MemoryAddress addr, MemoryAddress pc, unsigned long long aLogicalTime) {
    RepetReturnType ret = kRepetRetNone;
    long long group_sequence_no = 0;
    bool removedPht = false;
      ...
      if (! iter->second.hasIndex() ||  iter->second.last_poke != makeRepetIndex(addr, pc) ) {
        ...
        if(accessedThisGen.any()) {
          ...
          if(iter->second.finalized) {
            ...
            int hits = finalizeGroupRepet(index,
                                          iter->second.pattern,
                                          iter->second.fills,
                                          iter->second.offChipFills,
                                          removedPht);
  ...
  */
  ...
  int finalizeGroupRepet(RepetIndex index, SpatialPattern current, SpatialPattern fills, SpatialPattern offChipFills, bool & removed) {
    if (index == 0) {
    ...
    if(hist) {
      ...
      if(theRepetType == 1) {
        ...
        int actualFillHits = countBits(fillHits);
      ...
      if(countBits(fillHits) > 1) {
        //unsigned long long key = (index<<32) | current;
  ...
  bool movesRight(OrderingMap & order) {
    ...
    if(dirIter != order.endSeq()) {
      int blk1 = dirIter->first;
      dirIter++;
      if(dirIter != order.endSeq()) {
        int blk2 = dirIter->first;
        dirIter++;
        if(dirIter != order.endSeq()) {
          int blk3 = dirIter->first;
  ...
  void finalizeGroupOrdering(RepetIndex index, OrderingMap & orig, int fillHits, bool removed) {
    OrderingMap build;
    if(doRotate()) {
    ...
    int denseThreshold = getDenseThreshold();
    ...
    if(blockIter != history.end()) {
      int histSeq = blockIter->second;   // intentional conversion from unsigned char
      seqIter++;
      while(seqIter != build.endSeq()) {
        blockIter = history.find(seqIter->first);
        if(blockIter != history.end()) {
          int dist = (int)blockIter->second - histSeq;
    ...
    if(seqIter != build.endSeq()) {
      ...
      if(blockIter != sortedRight.end()) {
        ...
        int histSeq = blockIter->second;   // intentional conversion from unsigned char
        int hist2Seq = block2Iter->second;   // intentional conversion from unsigned char
        seqIter++;
        while(seqIter != build.endSeq()) {
          int dist, dist2;
          blockIter = sortedRight.find(seqIter->first);
  ...
  void doBufFetchGen(MemoryAddress addr, MemoryAddress pc, long long aSeqNo) {
    if(theSendStreams) {
  ...
  unsigned char makeNext(unsigned char curr, bool isRight) {
  ...
  void doStreamGen(MemoryAddress addr, MemoryAddress pc) {
    ...
    if(iter != theOrderingHistory.end()) {
      ...
      if(theStreamDense && ((int)order.size() > getDenseThreshold())) {
        ...
        int numBlocks = 0;
        unsigned char nextBlock = makeNext(stop, isRight);
  ...
  void doGroupTime(MemoryAddress addr, MemoryAddress pc, bool evict, bool beginend) {
    GroupAddress group = makeGroupAddress(addr, theSgpBits);
    unsigned long long curr = theFlexus->cycleCount();
  ...
  GroupAddress makeGroupAddress(MemoryAddress addr, int groupBits) {
    unsigned long offsetMask = (theBlockSize << groupBits) - 1;
    GroupAddress res = (addr & ~(offsetMask));
    //DBG_(Dev, ( << "makeGroupAddress: addr=" << std::hex << addr << " bits=" << groupBits << " res=" << res ) );
    return res;
  }

  SpatialPattern makeGroupPattern(MemoryAddress addr, int groupBits) {
    //SpatialPattern res = (1ULL << (unsigned long long)makeGroupOffset(addr,groupBits));
  ...
  GroupOffset makeGroupIndex(MemoryAddress addr, int groupBits) {
    GroupOffset res = addr >> (theBlockBits + groupBits);
    //DBG_(Dev, ( << "makeGroupIndex: addr=" << std::hex << addr << " bits=" << groupBits << " res=" << res ) );
    return res;
  }

  GroupOffset makeGroupOffset(MemoryAddress addr, int groupBits) {
  ...
  RepetIndex makeRepetIndex(MemoryAddress addr, MemoryAddress pc) {
    ...
    switch(theRepetType) {
      ...
      case 1:  // PC + block offset within SG
        ...
        res = (unsigned long long)pc << theSgpBits;
        res |= groupOffset;
        break;
      case 2:  // PC + address
        //res = ((unsigned long long)pc << 32) | (unsigned long long) makeBlockAddress(addr);
        res = ((unsigned long long) makeShiftedBlockAddress(addr) << thePcBits) | (unsigned long long)pc;
        break;
      ...
      case 4:  // PC + word offset within SG
        ...
        res = (unsigned long long)pc << theWordBits;
        res |= groupOffset;
        break;
      case 5:  // PC + byte offset within SG
        groupOffset = makeByteOffset(addr);
        res = (unsigned long long)pc << theByteBits;
        res |= groupOffset;
        break;
      case 6:  // PC + rotated block offset
  ...
  MemoryAddress extractPC(RepetIndex idx, int groupBits) {
  ...
  GroupOffset extractGroupOffset(RepetIndex idx, int groupBits) {
    DBG_Assert(theRepetType == 1);
  ...
  PhtIndex makePhtIndex(MemoryAddress addr, MemoryAddress pc, int groupBits) {
    GroupOffset groupOffset = makeGroupOffset(addr, groupBits);
    unsigned long long temp= ((unsigned long long)pc << groupBits) >> 2;
    temp |= groupOffset;
  ...
  /*
  unsigned long countBits(SpatialPattern pattern) {
  ...
  */
  ...
  unsigned long countBits(SpatialPattern pattern) {
    //DBG_(Dev, ( << "countBits: pattern=" << std::hex << pattern ) );
    unsigned long count = pattern.count();
    //DBG_(Dev, ( << "   res=" << std::dec << count ) );
    return count;
  }

  unsigned long mylog2(unsigned long num) {
    unsigned long ii = 0;
    while(num > 1) {
      ii++;
      num >>= 1;
  ...
  void saveState(std::ostream & ofs) {
    ...
    if(thePhtAssoc > 0) {
      ...
      std::map<unsigned long long,RepetIndex> repetOrder;
      RepetHistoryIter iter = theRepetAssocHistory.index_begin();
      while(iter != theRepetAssocHistory.index_end()) {
      ...
      std::map<unsigned long long,RepetIndex>::iterator iter2 = repetOrder.begin();
      unsigned long long lastAccess = 0;
