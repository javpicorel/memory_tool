...
struct SgpTrackStats {
  ...
  Stat::StatInstanceCounter<long long> EpochsPerGen;
  Stat::StatInstanceCounter<long long> OffchipEpochMembership;
  Stat::StatInstanceCounter<long long> HitsEpochMembership;
  Stat::StatInstanceCounter<long long> OffchipHeadMembership;
  Stat::StatInstanceCounter<long long> HitsHeadMembership;
  ...
  Stat::StatInstanceCounter<long long> DetailBothDist;
...
struct SgpTrackEntry {
  ...
  SgpTrackEntry(int sgpBlocks, int offset, bool hit)
  ...
  SgpTrackEntry(int sgpBlocks, SGvector & predictions)
    : theSeenAccesses(sgpBlocks)
    , theOffchipMisses(sgpBlocks)
  ...
  void offchipMiss(int offset) {
    if(theFirstMiss < 0) {
      theFirstMiss = offset;
    }
    theOffchipMisses.set(offset);
  }
  void sgpHit(int offset) {
    if(theFirstMiss < 0) {
      theFirstMiss = offset;
    }
    theOffchipMisses.set(offset);
  ...
  void addParallel(int head, SGvector & parallel, bool disp) {
  ...
  void endGen(SgpTrackStats & stats, bool disp) {
    ...
    for(iter = theParallel.begin(); iter != theParallel.end(); ++iter) {
      ...
      if(thisSgp.any()) {
        // some hits
        int sgpOnly = 1;  // core cannot get first miss in epoch
        int both = thisSgp.count() - 1;
        int coreOnly = thisMiss.count() - both - 1;
...
public:
  SgpTracker(std::string aName, int blockSize, int sgpBlocks)
    : theName(aName)
  ...
  void parallelList(address_t block, std::set<unsigned long long> & list) {
    address_t group = makeGroup(block);
    SgpTrackIter iter = theGroups.find(group);
    if(iter != theGroups.end()) {
      SGvector parallel(theSgpBlocks);
      std::set<unsigned long long>::iterator setIter = list.begin();
      for(; setIter != list.end(); ++setIter) {
        if(group == makeGroup(*setIter)) {
          int offset = makeOffset(*setIter);
          parallel.set(offset);
        }
      }
      int offset = makeOffset(block);
...
private:
  ...
  int makeOffset(address_t addr) {
    return ((addr & ~theGroupMask) >> theOffsetShift);
  }
  /*
  address_t makeGroup(unsigned long long addr) {
    return (addr & (unsigned long long)theGroupMask);
  }
  int makeOffset(address_t addr) {
    return ((addr & ~(unsigned long long)theGroupMask) >> theOffsetShift);
  }
  */
};
