...
namespace nuArch {
  ...
  MemoryPortArbiter::MemoryPortArbiter( CoreImpl & aCore, int aNumPorts, int aMaxStorePrefetches)
...
*/
  ...
  void MemoryPortArbiter::request( eOperation op, unsigned long long age, boost::intrusive_ptr<Instruction> anInstruction) {
    if (theCore.theInOrderMemory) {
  ...
  void CoreImpl::issue(boost::intrusive_ptr<Instruction> anInstruction ) {
    ...
    if (theEarlySGP) {
      ...
      unsigned long long logical_timestamp = theCommitNumber + theSRB.size() + distance;
  ...
  void CoreImpl::issueStore() {
    ...
    if (    ( ! theMemQueue.empty() )
      ...
      if ( theConsistencyModel == kRMO && ! theMemQueue.front().isAtomic() && theMemQueue.front().thePaddr && ! theMemQueue.front().theNonCacheable) {
        PhysicalMemoryAddress aligned = PhysicalMemoryAddress( static_cast<unsigned long long>(theMemQueue.front().thePaddr) & ~( theCoherenceUnit -1) );        
        if (theSBLines_Permission.find(aligned) != theSBLines_Permission.end()) {
          if (theSBLines_Permission[aligned].second ) {      
            ...
            ValueTracker::valueTracker().commitStore( theNode, op.thePAddr, op.theSize, value);
