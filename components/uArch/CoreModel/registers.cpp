...
*/
  ...
  void dec_mod8(unsigned int & anInt) {
    if (anInt == 0) {
      anInt = 7;
    } else {
      --anInt;
  ...
  void inc_mod8(unsigned int & anInt) {
    ++anInt;
    if (anInt >= 8 ) {
      anInt = 0;
    }
  ...
  unsigned int CoreImpl::saveWindow() {
  ...
  unsigned int CoreImpl::restoreWindow() {
  ...
  unsigned int CoreImpl::getCWP() {
    return theWindowMap.getCWP();
  }

  unsigned long long CoreImpl::getRRegister(unsigned int aReg) {
    unmapped_reg reg;
    reg.theType = rRegisters;
    reg.theIndex = aReg;
    mapped_reg reg_p = map(reg);
    register_value reg_val = readRegister( reg_p );
    return boost::get<unsigned long long>(reg_val);
  }

  void CoreImpl::setRRegister(unsigned int aReg, unsigned long long aVal) {
    unmapped_reg reg;
    reg.theType = rRegisters;
    reg.theIndex = aReg;
    mapped_reg reg_p = map(reg);
  ...
  void CoreImpl::setASI(unsigned long long aVal) {
    return setRRegister(kRegASI, aVal);
  }

  unsigned long long CoreImpl::getASI() {
    return getRRegister(kRegASI);
  }

  void CoreImpl::setCCR(unsigned long aValue) {
    unmapped_reg reg;
    reg.theType = ccBits;
    reg.theIndex = 0;
    mapped_reg reg_p = map(reg);
  ...
  unsigned long CoreImpl::getCCRArchitectural() {
    mapped_reg mreg;
    mreg.theType = ccBits;
    ...
    void CoreImpl::writeFSR( unsigned long long anFSR) {
      theFSR = anFSR;
    ...
    unsigned long long CoreImpl::readFSR() {
      unsigned long long fsr = theFSR;
      unmapped_reg reg;
      reg.theType = ccBits;
      ...
      try { 
        ...
        //debugging
        /*
        DBG_(VVerb, ( << "nikos: boost::get<> fails" ));
        void * array[25];
        int nSize = backtrace(array, 25);
        char ** symbols = backtrace_symbols(array, nSize);
        for (int i = 0; i < nSize; i++) {
          DBG_( VVerb, ( << symbols[i] ));
        };
        std::free(symbols);
        */
        ...
        unsigned long long fcc0_ull = ( fcc0.to_ulong() & 3);
        unsigned long long fcc1_ull = ( fcc1.to_ulong() & 3);
        unsigned long long fcc2_ull = ( fcc2.to_ulong() & 3);
        unsigned long long fcc3_ull = ( fcc3.to_ulong() & 3);
        //Mask out all FCC fields
        fsr &= 0xFFFFFFC0FFFFF3FFULL;
        fsr |= (fcc0_ull << 10) | (fcc1_ull << 32) | (fcc2_ull << 34) | (fcc3_ull << 36);
      } catch (...) {
        DBG_(Dev, ( << "readFSR() through an exception.  return an incorrect value and let validation clean it up." ) ); 
      }
      return fsr;
    }


    int CoreImpl::selectedRegisterSet() const {
    ...
    void CoreImpl::getv9State( v9State & aState) {
      ...
      for (int i = 0; i < 32; ++i) {
        reg.theIndex = i;
        aState.theGlobalRegs[i] = boost::get<unsigned long long>(readArchitecturalRegister( reg, false ));
      }
      for (int i = 0 ; i < 8*16; ++i) {
        reg.theIndex = i+32;
        aState.theWindowRegs[i] = boost::get<unsigned long long>(readArchitecturalRegister( reg, false ));
      }

      reg.theType = fRegisters;
      for (int i = 0 ; i < 64; ++i) {
        reg.theIndex = i;
        aState.theFPRegs[i] = boost::get<unsigned long long>(readArchitecturalRegister( reg, false ));
      }
      reg.theType = rRegisters;
      ...
      aState.theASI = boost::get<unsigned long long>(readArchitecturalRegister( reg, false ));
      reg.theIndex = kRegGSR;
      aState.theGSR = boost::get<unsigned long long>(readArchitecturalRegister( reg, false ));
      aState.theTL = getTL();
      ...
      reg.theIndex = kRegY;
      aState.theY = boost::get<unsigned long long>(readArchitecturalRegister( reg, false ));
    ...
    void CoreImpl::restorev9State( v9State & aState) {
      ...
      unsigned long long fsr = aState.theFSR;
