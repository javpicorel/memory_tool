...
namespace nuArch {
  ...
  void CoreImpl::invalidate( PhysicalMemoryAddress anAddress) {
    FLEXUS_PROFILE();
    DBG_Assert ( static_cast<unsigned long long>( anAddress) == ( static_cast<unsigned long long>(anAddress) & ~( theCoherenceUnit -1) ) );
    ...
    //Loads which meet ALL the following conditions must be squashed:
      ...
      //operation.
      boost::optional<unsigned long long> first_incomplete;
      if (isSpeculating()) {
  ...
  bool CoreImpl::processReply( MemOp const & anOperation ) {
    ...
    PhysicalMemoryAddress addr( static_cast<unsigned long long>(anOperation.thePAddr) & ~( theCoherenceUnit -1));
