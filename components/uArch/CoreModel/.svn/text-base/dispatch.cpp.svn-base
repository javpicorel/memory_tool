...
namespace nuArch {
  ...
  int CoreImpl::availableROB() const {
    ...
    if (anInsn->haltDispatch()) {
      ...
      if ( theROB.empty() && theEmptyROBCause == kMispredict ) {
           ...
           )
        {
          ...
          const unsigned long long theAvgMispredictCycles  = (theMispredictCount > 0 ? static_cast<unsigned long long>(theMispredictCycles * 1.0 / theMispredictCount + 0.5) : 0);
          ...
          const unsigned long long theCurrMispredictCycles = theTimeBreakdown.subtTmpStallCycles( theAvgMispredictCycles );
