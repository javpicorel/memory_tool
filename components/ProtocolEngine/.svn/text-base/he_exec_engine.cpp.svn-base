...
void tHEExecEngine::handle_HE_INSTR_SEND(tThread aThread, const unsigned long args) {
  ...
  if (dest == 2 /* ToSharers */) {
    ...
    for (unsigned int i=0; i < theSrvcProv.numNodes(); i++) {
...
void tHEExecEngine::handle_HE_INSTR_SEND_FWD(tThread aThread, const unsigned long args) {
  // NOTE: they should match xe.mdef argument fields
...
void tHEExecEngine::handle_HE_INSTR_RECEIVE(tThread aThread, const unsigned long args) {
...
void tHEExecEngine::handle_HE_INSTR_TEST(tThread aThread, const unsigned long args) {
  // NOTE: they should match xe.mdef argument fields
...
void tHEExecEngine::handle_HE_INSTR_SET(tThread aThread, const unsigned long args) {
  // NOTE: they should match xe.mdef argument fields
...
std::pair<tCpuOpType,std::string> determineHECPUOp( unsigned long args ) {
  ...
  switch(op) {
    ...
    default:
      DBG_Assert( false,  ( << "Invalid HE CPU_OP: " << args) ); return std::make_pair(CPU_INVALIDATE, std::string("Invalidate"));
  }
}

void tHEExecEngine::handle_HE_INSTR_CPU_OP(tThread aThread, const unsigned long args) {
...
void tHEExecEngine::handle_HE_INPUT_Q_OP(tThread aThread, const unsigned long args) {
...
void tHEExecEngine::deliverReply(tThread aThread, tMessageType aType) {
  const unsigned internal_type = map_MessageType_to_EngineMessageType(aType, HE_INSTR_RECEIVE);
...
unsigned tHEExecEngine::map_MessageType_to_EngineMessageType(tMessageType type, const unsigned instr) const
{
  switch (type) {
    ...
    case ReadReq:                break;                               // VC 0
    case WriteReq:               break;                               // VC 0
    case UpgradeReq:             break;                               // VC 0
    case FlushReq:               return HE_RX_FlushReq;               // VC 1 Contains data
    case WritebackReq:           return HE_RX_WritebackReq;           // VC 1 Contains data
    // case ReadExReq:           // - Not used
    // case WriteAllReq:         // - Not used
    // case ReadAndForgetReq:    // - Not used

    // Requests From Directory to Cache
    case LocalInvalidationReq:   return HE_TX_LocalInvalidationReq;   // VC 1
    case RemoteInvalidationReq:  return HE_TX_RemoteInvalidationReq;  // VC 1
    case ForwardedReadReq:       return HE_TX_ForwardedReadReq;       // VC 1
    case ForwardedWriteReq:      return HE_TX_ForwardedWriteReq;      // VC 1
    case RecallReadReq:          return HE_TX_RecallReadReq;          // VC 1
    case RecallWriteReq:         return HE_TX_RecallWriteReq;         // VC 1

    // Replies From Cache to Directory
    case ForwardedWriteAck:      return HE_RX_ForwardedWriteAck;      // VC 2
    case ForwardedReadAck:       return HE_RX_ForwardedReadAck;       // VC 2 Contains data
    case RecallReadAck:          return HE_RX_RecallReadAck;          // VC 2 Contains data
    case RecallWriteAck:         return HE_RX_RecallWriteAck;         // VC 2 Contains data
    case LocalInvalidationAck:   return HE_RX_LocalInvalidationAck;   // VC 2

    // Replies From Directory to Cache
    case ReadAck:                return HE_TX_ReadAck;                // VC 1 Contains data
    case WriteAck:               return HE_TX_WriteAck;               // VC 1 Contains data and #inval acks to expect
    case UpgradeAck:             return HE_TX_UpgradeAck;             // VC 1 #inval acks to expect
    case WritebackAck:           return HE_TX_WritebackAck;           // VC 2
    case WritebackStaleRdAck:    return HE_TX_WritebackStaleRdAck;    // VC 2
    case WritebackStaleWrAck:    return HE_TX_WritebackStaleWrAck;    // VC 2
    case FlushAck:               return HE_TX_FlushAck;               // VC 1

    // Replies From Cache to Cache
    case RemoteInvalidationAck:  break;                               // VC 2
    case ReadFwd:                break;                               // VC 2 Contains Data
    case WriteFwd:               break;                               // VC 2 Contains Data

    // Replies from Caches to PE
    case InvAck:                 return HE_RX_InvAck;                // The cache acknowledges an Invalidate CPU_OP
    case InvUpdateAck:           return HE_RX_InvUpdateAck;          // Contains data. The cache acknowledges an Invalidate CPU_OP.
    case DowngradeAck:           return HE_RX_DowngradeAck;          // The cache acknowledges a Downgrade CPU_OP
    case DowngradeUpdateAck:     return HE_RX_DowngradeUpdateAck;    // Contains data. The cache acknowledges a Downgrade CPU_OP.
...
tMessageType tHEExecEngine::map_EngineMessageType_to_MessageType(unsigned type, const unsigned instr) const {
  switch (instr) {
    case HE_INSTR_SEND:
      switch (type) {
        // Replies From Directory to Cache
        case HE_TX_ReadAck:                return ReadAck;                // VC 1 Contains data
        case HE_TX_WriteAck:               return WriteAck;               // VC 1 Contains data and #inval acks to expect
        case HE_TX_UpgradeAck:             return UpgradeAck;             // VC 1 #inval acks to expect
        case HE_TX_WritebackAck:           return WritebackAck;           // VC 2
        case HE_TX_FlushAck:               return FlushAck;               // VC 1

        // Error in protocol
        case HE_TX_Error:                  return ProtocolError;          // ERROR IN PROTOCOL!!!
    ...
    case HE_INSTR_SEND_FWD:
      switch (type) {
        ...
        // Requests From Directory to Cache
        case HE_TX_LocalInvalidationReq:   return LocalInvalidationReq;   // VC 1
        case HE_TX_RemoteInvalidationReq:  return RemoteInvalidationReq;  // VC 1
        case HE_TX_ForwardedReadReq:       return ForwardedReadReq;       // VC 1
        case HE_TX_ForwardedWriteReq:      return ForwardedWriteReq;      // VC 1
        case HE_TX_RecallReadReq:          return RecallReadReq;          // VC 1
        case HE_TX_RecallWriteReq:         return RecallWriteReq;         // VC 1

        // Replies From Directory to Cache
        case HE_TX_WritebackStaleRdAck:    return WritebackStaleRdAck;    // VC 2
        case HE_TX_WritebackStaleWrAck:    return WritebackStaleWrAck;    // VC 2

        // Error in protocol
        case HE_TX_FWD_Error:              return ProtocolError;          // ERROR IN PROTOCOL!!!
