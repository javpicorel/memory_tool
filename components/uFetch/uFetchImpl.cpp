...
/*
	if (load_tag != 0){
		DBG_ (Dev, (<< "MAMMAD: tag: " << load_tag << " vlid: " << load_state) );
	}
*/
        if (load_state & kSave_ValidBit) {
	  //if (load_tag != 0){
	    //DBG_ (Dev, (<< "MAMMAD: put in tag: " << load_tag << " vlid: " << load_state) );
	  //}
          //theCache.insert( std::make_pair((load_tag << tagShift) | i,0) );
          theCache.insert( std::make_pair((load_tag),0) );
	  //DBG_ (Dev, (<< "MAMMAD: tag: " << load_tag << " vlid: " << load_state << " tag shift: "<< tagShift) );
	  //DBG_ (Dev, (<< "MAMMAD: put in : " << ((load_tag << tagShift) | i)) );
          unsigned long long ttt = (load_tag << tagShift) | i;
          //DBG_ (Dev, (<< "MAMMAD: put in ttt : " << (ttt)) );
  ...
  bool lookup(unsigned long long addr) {
    addr = addr >> theCacheBlockShift;
    SimCacheIter iter = theCache.find(addr);
    //DBG_( Dev, ( << "MAMMAD: looking up for addr " << addr ));
    if(iter != theCache.end()) {
      theCache.move_back(iter);
      return true;  // present
    }
...
*/
  ...
  private:
    ...
    bool icacheLookup( index_t anIndex, VirtualMemoryAddress vaddr ) {
      ...
      //DBG_( Dev, Comp(*this) ( << "MAMMAD: CPU[" << std::setfill('0') << std::setw(2) << flexusIndex() << "." << anIndex <<"] fetch paddr " << paddr ));

      bool hit = lookup(paddr);
      ++theFetchAccesses;
      if (hit) {
        //DBG_( Dev, Comp(*this) ( << "MAMMAD: HIT" ));
        ++theHits;
      ...
      //DBG_( Dev, Comp(*this) ( << "MAMMAD: CPU[" << std::setfill('0') << std::setw(2) << flexusIndex() << "." << anIndex <<"] L1I MISS " << vaddr << " " << *theIcacheMiss[anIndex]));
