...
const unsigned long kLargeMemoryQueue = 16000;
...
class SimicsTracerImpl {
    ...
    boost::function< void(int, MemoryMessage &, unsigned long) > toL1I;
  ...
  public:
    ...
    API::conf_object_t * cpu() const { return theCPU; }

    // Initialize the tracer to the desired CPU
    void init(API::conf_object_t * aCPU, index_t anIndex, boost::function< void(int, MemoryMessage &) > aToL1D, boost::function< void(int, MemoryMessage &, unsigned long) > aToL1I, boost::function< void(int, MemoryMessage &) > aToNAW) {
      theCPU = aCPU;
      theIndex = anIndex;
      toL1D = aToL1D;
      toL1I = aToL1I;
    ...
    API::cycles_t insn_fetch(Simics::APIFwd::memory_transaction_t *mem_trans) {
      ...
      unsigned long opcode = API::SIM_read_phys_memory(theCPU, mem_trans->s.physical_address, 4);
    ...
    API::cycles_t trace_mem_hier_operate(API::conf_object_t *space, API::map_list_t *map, API::generic_transaction_t *aMemTrans) {
      ...
      if (mem_trans->s.atomic) {
        //Need to determine opcode, as this may be an RMW or CAS
          ...
          unsigned long op_code = API::SIM_read_phys_memory(theCPU, pc, 4);
...
class SimicsTracer : public Simics::AddInObject <SimicsTracerImpl> {
   ...
   public:
    ...
    static std::string className() { return "DecoupledFeeder"; }
...
class SimicsTracerManagerImpl : public SimicsTracerManager {
  ...
  public:
    SimicsTracerManagerImpl(int aNumCPUs, boost::function< void(int, MemoryMessage &) > aToL1D, boost::function< void(int, MemoryMessage &, unsigned long) > aToL1I, boost::function< void(MemoryMessage &) > aToDMA, boost::function< void(int, MemoryMessage &) > aToNAW)
     : theNumCPUs(aNumCPUs)
    ...
    void setSimicsQuantum(long long aSwitchTime) {
      API::attr_value_t attr;
    ...
    void enableInstructionTracing() {
      ...
      for (int i = 0; i < theNumCPUs; ++i) {
        ...
        switch (errorCode) {
          case API::Sim_Set_Ok:                  DBG_ ( Crit, ( << ">>>>> OK" )); break;
          case API::Sim_Set_Need_Integer:        DBG_ ( Crit, ( << ">>>>> Need_Integer" )); break;
          case API::Sim_Set_Need_Floating:       DBG_ ( Crit, ( << ">>>>> Need_Floating" )); break;
          case API::Sim_Set_Need_String:         DBG_ ( Crit, ( << ">>>>> Need_String" )); break;
          case API::Sim_Set_Need_List:           DBG_ ( Crit, ( << ">>>>> Need_List" )); break;
          case API::Sim_Set_Need_Data:           DBG_ ( Crit, ( << ">>>>> Need_Data" )); break;
          case API::Sim_Set_Need_Object:         DBG_ ( Crit, ( << ">>>>> Need_Object" )); break;
          case API::Sim_Set_Object_Not_Found:    DBG_ ( Crit, ( << ">>>>> Object_Not_Found" )); break;
          case API::Sim_Set_Interface_Not_Found: DBG_ ( Crit, ( << ">>>>> Interface_Not_Found" )); break;
          case API::Sim_Set_Illegal_Value:       DBG_ ( Crit, ( << ">>>>> Illegal_Value" )); break;
          case API::Sim_Set_Illegal_Type:        DBG_ ( Crit, ( << ">>>>> Illegal_Type" )); break;
          case API::Sim_Set_Illegal_Index:       DBG_ ( Crit, ( << ">>>>> Illegal_Index" )); break;
          case API::Sim_Set_Attribute_Not_Found: DBG_ ( Crit, ( << ">>>>> Attribute_Not_Found" )); break;
          case API::Sim_Set_Not_Writable:        DBG_ ( Crit, ( << ">>>>> Not_Writable" )); break;
          case API::Sim_Set_Ignored:             DBG_ ( Crit, ( << ">>>>> Ignored" )); break;
          default:
            DBG_Assert ( false );
        }
      }
  ...
  private:
    ...
    void createTracers() {
      ...
      const int max_simics_cpu_no = 384;
      for(int ii = 0, simics_cpu_no = 0; ii < theNumCPUs ; ++ii, ++simics_cpu_no) {
...
SimicsTracerManager * SimicsTracerManager::construct(int aNumCPUs, boost::function< void(int, MemoryMessage &) > toL1D, boost::function< void(int, MemoryMessage &, unsigned long) > toL1I, boost::function< void(MemoryMessage &) > toDMA, boost::function< void(int, MemoryMessage &) > toNAW) {
