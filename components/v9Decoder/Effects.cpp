...
struct BranchFeedbackWithOperandEffect: public Effect {
  ...
  void invoke(SemanticInstruction & anInstruction) {
    ...
    VirtualMemoryAddress target(anInstruction.operand< unsigned long long > (theOperandCode));
...
struct ReturnFromTrapEffect: public Effect {
  ...
  void invoke(SemanticInstruction & anInstruction) {
    ...
    int tl = anInstruction.core()->getTL();
    if (tl >= 5 || tl < 0) {
...
struct BranchConditionallyEffect: public Effect {
  ...
  BranchConditionallyEffect( VirtualMemoryAddress aTarget, bool anAnnul, unsigned int aCondition, bool floating, eOperandCode aCCRCode)
    : theTarget(aTarget)
    , theAnnul(anAnnul)
...
struct BranchRegConditionallyEffect: public Effect {
  ...
  BranchRegConditionallyEffect( VirtualMemoryAddress aTarget, bool anAnnul, unsigned int aCondition, eOperandCode aValueCode)
    : theTarget(aTarget)
    , theAnnul(anAnnul)
    , theCondition(aCondition)
  ...
  void invoke(SemanticInstruction & anInstruction) {
    FLEXUS_PROFILE();
    unsigned long long val = anInstruction.operand< unsigned long long > (theValueCode);
...
int regWinTrapType(bool isFill, SemanticInstruction & anInstruction ) {
  int tt;
  if (isFill) {
    tt = 0xC0; /* fill */
  } else {
...
struct SaveWindowEffect: public Effect {
  void invoke(SemanticInstruction & anInstruction) {
    FLEXUS_PROFILE();
    unsigned long long cwp = anInstruction.core()->saveWindow();
...
struct RestoreWindowEffect: public Effect {
  void invoke(SemanticInstruction & anInstruction) {
    FLEXUS_PROFILE();
    unsigned long long cwp = anInstruction.core()->restoreWindow();
    anInstruction.setOperand(kocCWP, cwp);
...
struct ReadPREffect: public Effect {
  ...
  void invoke(SemanticInstruction & anInstruction) {
    ...
    if (! anInstruction.isAnnulled()) {
      unsigned long long pr = anInstruction.core()->readPR(thePR);
...
struct WritePREffect: public Effect {
  unsigned int thePR;
  WritePREffect( unsigned int aPR)
    : thePR(aPR)
    {}
  ...
  void invoke(SemanticInstruction & anInstruction) {
    FLEXUS_PROFILE();
    if (! anInstruction.isAnnulled()) {
      unsigned long long pr = anInstruction.operand< unsigned long long > (kResult);
...
struct IMMUExceptionEffect: public Effect {
  ...
  void invoke(SemanticInstruction & anInstruction) {
    ...
    int exception = anInstruction.retryTranslation();
    if (exception != 0) {
...
struct TccEffect: public Effect {
  void invoke(SemanticInstruction & anInstruction) {
    ...
    unsigned long long val = anInstruction.operand< unsigned long long > (kResult);
    if (val == 0) {
...
struct UpdateFPRSEffect: public Effect {
  unsigned int theDestFPReg;
  UpdateFPRSEffect( unsigned int aDestReg)
  ...
  void invoke(SemanticInstruction & anInstruction) {
    ...
    if (! anInstruction.isAnnulled()) {
      ...
      unsigned long long fprs = anInstruction.core()->getFPRS();
      if (theDestFPReg < 32) {
        fprs |= 1;  //set DL
...
struct WriteFPRSEffect: public Effect {
  ...
  void invoke(SemanticInstruction & anInstruction) {
    FLEXUS_PROFILE();
    if (! anInstruction.isAnnulled()) {
      unsigned long long fprs = anInstruction.operand< unsigned long long > (kResult);
...
struct ReadFPRSEffect: public Effect {
  ...
  void invoke(SemanticInstruction & anInstruction) {
    ...
    if (! anInstruction.isAnnulled()) {
      unsigned long long val = anInstruction.core()->getFPRS();
      mapped_reg name = anInstruction.operand< mapped_reg > (kPD);
...
struct WriteFSREffect: public Effect {
  ...
  void invoke(SemanticInstruction & anInstruction) {
    FLEXUS_PROFILE();
    if (! anInstruction.isAnnulled()) {
      unsigned long long fsr = anInstruction.operand< unsigned long long > (kResult);
      DBG_( Verb, ( << anInstruction << " Write FSR value= " << std::hex << fsr << std::dec ) );

      if (theSize == kWord) {
        unsigned long long current_fsr = anInstruction.core()->readFSR();
...
struct StoreFSREffect: public Effect {
  ...
  void invoke(SemanticInstruction & anInstruction) {
    FLEXUS_PROFILE();
    if (! anInstruction.isAnnulled()) {
      unsigned long long fsr = anInstruction.core()->readFSR();
      anInstruction.setOperand(kocFSR, fsr );
      if (theSize == kWord) {
        fsr &= 0xFFFFFFFFULL;
      }
