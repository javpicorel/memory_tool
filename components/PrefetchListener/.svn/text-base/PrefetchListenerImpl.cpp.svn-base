...
using boost::intrusive_ptr;
  ...
  Stat::StatInstanceCounter<long long> thePBHits_User("sys-PBHits:User");
  Stat::StatInstanceCounter<long long> thePBHits_System("sys-PBHits:System");
...
struct PLStats {
  ...
  Stat::StatInstanceCounter<long long> statHitInstances_User;
  Stat::StatInstanceCounter<long long> statHitInstances_System;
...
struct PrefetchEntry {
  ...
  PrefetchEntry( MemoryAddress anAddress, int aQueue)
...
struct PrefetchQueueEntry {
  MemoryAddress theAddress;
  unsigned long long theInsertTime;
  PrefetchQueueEntry(MemoryAddress anAddress, unsigned long long anInsertTime)
    : theAddress(anAddress)
...
struct PrefetchQueue {
  int theId; //Matches offset in thePrefetchQueues
  unsigned long long theSequenceNumber; //Tracks the total number of queues allocated

  pb_queue_t theQueue[2]; //The two address queues
  bool theQueueValid[2]; //Valid bits for each address queue
  int theHotQueue; //Indication of which address queue is "hot"

  long long theTag[2]; //Source tag
  int theSource[2]; //Source node
  long theFirstLocation[2]; //Start location
  long theNextLocation[2]; //Next location
  unsigned int theHalfListThreshold[2]; //Threshold for requesting more address bits
  ...
  int theCredits;
  int theBufferedLines; //Number of lines this queue has in the PB
  unsigned long long theMostRecentHit; //Timestamp of most recent hit

  bool theWatching[2];
  bool theDegenerate;

  long theHits;  //Number of hits produced by this queue pair
  long theCompletedPrefetches; //Prefetches completed by this pair
  long theAddressLists; //Address lists received by this pair
  long theRedundants;  //Number of blocks from this queue that have been
...
public:
  ...
  bool isValid() const { return theQueueValid[0] || theQueueValid[1]; }
  bool isValid(int aSlot) const { return theQueueValid[aSlot]; }
  int bufferedLines() const { return theBufferedLines; }
  int credits() const { if (isValid()) { return theCredits; } else { return 0; } }
  int value() const {
    if (isValid()) {
      int score = theCredits + theBufferedLines;
      if (isWatching(0)) {
        score++;
      }
      if (isWatching(1)) {
  ...
  unsigned long long mostRecentHit() const { return theMostRecentHit; }

  unsigned source(int aSlot) const { return theSource[aSlot]; }
  long long tag(int aSlot) const { return theTag[aSlot]; }
  long location(int aSlot) const { return theNextLocation[aSlot]; }
  long startLocation(int aSlot) const { return theFirstLocation[aSlot]; }
  MemoryAddress head(int aSlot) const { DBG_Assert( theQueueValid[aSlot] && !theQueue[aSlot].empty()); return theQueue[aSlot].front().theAddress; }
  void popHead(int aSlot) { DBG_Assert( theQueueValid[aSlot] && !theQueue[aSlot].empty()); theQueue[aSlot].pop_front(); }
  void setWatching(int aSlot, bool aFlag) { theWatching[aSlot] = aFlag; }
  bool isWatching(int aSlot) const { if (theQueueValid[aSlot]) { return theWatching[aSlot]; } else { return false ; } }

  bool isWatching(int aSlot, MemoryAddress anAddress) const {
  ...
  void changeCredit( int aCreditChange ) {
    theCredits += aCreditChange;
  }

  bool wantsMoreAddresses(int aSlot, unsigned int aThreshold) const {
  ...
  void finalize( PLStats & theStats ) {
    theStats.statHitsByLength << std::make_pair(static_cast<long long>(theHits), static_cast<long long>(theHits));
  ...
  bool join(PLStats & theStats, PrefetchCommand & cmd) {
    ...
    int requested_slot = (cmd.tag() >> 4) & 1;
  ...
  void allocate(PLStats & theStats, PrefetchCommand & cmd, unsigned long long aQueueSequence) {
  ...
  void addressListRequested(int aSlot) {
...
class FLEXUS_COMPONENT(PrefetchListener) {
  ...
  unsigned long long theQueueSequence;

  int theNextPrefetchQueue;
  int theTotalCredit;
 ...
 public:
  ...
  void initialize() {
    ...
    for (unsigned int i = 0; i < cfg.NumQueues; ++i) {
  ...
  void removeCredit(PrefetchQueue & q, int credit = 1) {
  ...
  void prefetch( MemoryAddress anAddress, int aQueue) {
    FLEXUS_PROFILE();
  ...
  void requestAddressList(PrefetchQueue & q, int aSlot ) {
    FLEXUS_PROFILE();
  ...
  void checkQueues() {
    ...
    for (unsigned int i = 0; i < thePrefetchQueues.size(); ++i) {
  ...
  void issuePrefetches() {
    ...
    int queue = theNextPrefetchQueue;
  ...
  void clipAndAllocate( PrefetchCommand & cmd ) {
    ...
    long new_end = cmd.location();
    long new_start = cmd.location() - cmd.addressList().size();
    ...
    for (unsigned int i = 0; i < thePrefetchQueues.size(); ++i) {
      ...
      for (int slot = 0; slot < 2; ++slot) {
        ...
        long existing_start = q.startLocation(slot);
        long existing_end = q.location(slot);
  ...
  int selectVictim() {
    FLEXUS_PROFILE();
    int victim = 0;
    for (unsigned int i = 1; i < thePrefetchQueues.size(); ++i) {
...
FLEXUS_PORT_ARRAY_WIDTH( PrefetchListener, FromNic) { return cfg.VChannels; }
