...
result_path="/mnt/parsafiler1/vol3/homedirs/picorel/MICRO2017/results/memory/"
memory_sizes=( 8 8 8 8 8 8 8 8 
               8 8 8 8 8 8 8 8
               8 8 8 8 8 8 8
             )
memory_tool_sizes=( 64 64 64 64 64 64 12 64
                    64 64 64 64 64 64 12 64
                    64 64 64 64 64 64 12
                  )
number_processes=( 1 2 4 8 )
#number_processes=( 8 )
associativity=( 1 2 4 8 16 32 64 128 256 512 1024 2048 4096 8192 16384 32768 65536 131072 262144 524288 1048576 2097152)
#operations=-1
  ...
  do
    ...
    do
      ...
      then
        if [ $j -eq 4 ] 
        then
          ./Fast3DCacheImpl $((1024*${memory_tool_sizes[$i]})) $i $page_size $k $cores $operations $workload_path${workload_name[$i]}"/" $result_path $((${memory_sizes[$i]}/$j)) $j $scramble > $result_path${workload_name[$i]}"_"$j"x_"$((${memory_sizes[$i]}/$j))"GB_Trace_"${memory_tool_sizes[$i]}"GB_Memory_"$k"ways_"$scramble"_scramble" &
        fi
      else #Not mix
        ./Fast3DCacheImpl $((1024*${memory_tool_sizes[$i]})) $i $page_size $k $cores $operations $workload_path${workload_name[$i]}"/" $result_path $((${memory_sizes[$i]}/$j)) $j $scramble > $result_path${workload_name[$i]}"_"$j"x_"$((${memory_sizes[$i]}/$j))"GB_Trace_"${memory_tool_sizes[$i]}"GB_Memory_"$k"ways_"$scramble"_scramble" &
      fi
