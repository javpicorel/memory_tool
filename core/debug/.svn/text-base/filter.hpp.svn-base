...
class OpFilter : public SimpleFilter {
  ...
  public:
    ...
    struct LongLong {
        ...
        long long extract(Entry const & anEntry, std::string const & aField) { return anEntry.getNumeric(aField); }
        std::string toString(long long const & aValue) { return boost::lexical_cast<std::string>(aValue); }
    };

    struct OpEqual {
      std::string const & toString() { static std::string str("=="); return str; }
      template <class T>
