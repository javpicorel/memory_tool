...
namespace Dbg {
  ...
  void ConsoleLogAction::process(Entry const & anEntry) {
    theFormat->format(std::cerr, anEntry); std::cerr.flush();
  }
  ...
  SeverityAction::SeverityAction(unsigned int aSeverity)
  ...
  class SaveBufferManager {
    ...
    public:
      ...
      void create(std::string const & aBufferName, unsigned int aSize) {
  ...
  SaveAction::SaveAction(std::string aBufferName, unsigned int aCircularBufferSize)
