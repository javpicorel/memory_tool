...
namespace Dbg {
  ...
  Entry::Entry(Severity aSeverity, char const * aFile, long aLine, char const * aFunction, long aGlobalCount, long long aCycleCount)
  ...
  Entry & Entry::set(std::string const & aFieldName, long long aFieldValue) {
    std::set<Field>::iterator iter(theFields.insert(aFieldName).first);
    iter->setValue(aFieldValue);
    return *this;
  }
  ...
  long long Entry::getNumeric(std::string aFieldName) const  {
    std::set<Field>::const_iterator iter = theFields.find(aFieldName);
    if (iter != theFields.end()) {
