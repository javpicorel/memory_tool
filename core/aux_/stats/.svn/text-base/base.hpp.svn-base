...
namespace aux_ {
  ...
  struct StatValueBase : public boost::counted_base {
    ...
    public:
      ...
      virtual long long asLongLong() const { return 0; };
      ...
      virtual boost::intrusive_ptr<StatValueBase> sumAccumulator() { throw 1; /* by default, stat's don't support sum accumulation */ }
      virtual void reduceSum(StatValueBase const & anRHS) { };
      virtual boost::intrusive_ptr<StatValueBase> avgAccumulator() { throw 1; /* by default, stat's don't support average accumulation */ }
      virtual void reduceAvg(StatValueBase const & anRHS) { };
      virtual boost::intrusive_ptr<StatValueBase> stdevAccumulator() { throw 1; /* by default, stat's don't support standard deviation accumulation */ }
  ...
  struct StatValueArrayBase : public StatValueBase {
    ...
    public:
      virtual StatValueBase & operator[](int anIndex) = 0;
      virtual void newValue(accumulation_type aValueType) = 0;
      virtual std::size_t size() = 0;
  };

  template<class Archive>
  void save(Archive & ar, ::boost::intrusive_ptr<StatValueBase> const & ptr, unsigned int version);

  template<class Archive>
  void load(Archive & ar, ::boost::intrusive_ptr<StatValueBase> & ptr, unsigned int version);

  template<class Archive>
  void save(Archive & ar, ::boost::intrusive_ptr<StatValueArrayBase> const & ptr, unsigned int version);

  template<class Archive>
  void load(Archive & ar, ::boost::intrusive_ptr<StatValueArrayBase> & ptr, unsigned int version);
...
namespace aux_ {
  ...
  class StatValueHandle : public StatValueHandle_Base {
    ...
    public:
      ...
      long long asLongLong(std::string const & options);
      double asDouble();
