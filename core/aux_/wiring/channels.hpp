...
struct resolve_channel<ToInstance, ToPort, push, true> {
  static bool invoke_available(Flexus::Core::index_t anIndex) {
     int width = ToInstance::iface::width(ToInstance::getInstance().theConfiguration, ToPort());
...
struct port_impl<push, PortName, AvailableFn, ManipulateFn > {
  ...
  port_impl(AvailableFn anAvail, ManipulateFn aPush, int anIndex)
   : theIndex(anIndex)
   , theAvail(anAvail)
   , thePush(aPush)
   {}
...
struct port_impl<pull, PortName, AvailableFn, ManipulateFn > {
  ...
  port_impl(AvailableFn anAvail, ManipulateFn aPull, int anIndex)
   : theIndex(anIndex)
   , theAvail(anAvail)
   , thePull(aPull)
   {}
...
struct port : public port_impl<typename Port::port_type, Port, AvailableFn, ManipulateFn > {
  port(AvailableFn avail, ManipulateFn manip, int anIndex)
