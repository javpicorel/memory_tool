...
namespace {
    ...
    FLEXUS_DECLARE_COMPONENT_PARAMETERS(Core,
        FLEXUS_PARAMETER( MaxCycles, unsigned long long, "Max Cycles", "max", 0 )
        FLEXUS_PARAMETER( SystemWidth, unsigned int, "System Width", "width", 1 )
    );
...
int main(int argc, char ** argv) {
    ...
    try {
      for (unsigned long long i = 0; CoreCfg.cfg().MaxCycles == 0 || i < CoreCfg.cfg().MaxCycles; ++i ) {
